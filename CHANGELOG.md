# Changelog

All notable changes follow Keep a Changelog conventions. Versions use Semantic
Versioning.

## [0.1.0] - 2026-07-18

### Added

- Verified offline Dictionary v2 from GrammarDB `RELEASE-202601`.
- Exact-offset Unicode homograph scanning.
- Dictionary list, show, detect, path, and prompt CLI commands.
- Full structured adjudication prompt contract.
- Lean choice-only prompt and host-side response decoder for small LLMs.
- Source-backed coarse POS, morphology features, meanings, and raw tags.
- Reproducible uv environment, strict CI, release workflow, and wheel smoke test.

[0.1.0]: https://github.com/idegterov/homograph-bel/releases/tag/v0.1.0
